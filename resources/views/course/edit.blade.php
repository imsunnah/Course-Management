...
<div class="container mt-5">
   ...
   <div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="card-header bg-secondary text-white py-3 fw-semibold">
                 Course Information
            </div>            <div class="card-body row g-3">
                <div class="col-md-8">
                ...
                <div class="card mb-3 shadow-sm module-item" data-module-id="{{ $module->id }}">
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input type="text" name="modules[{{ $module->id }}][title]" class="form-control" value="{{ $module->title }}" required>
                        </div>
                        ...
                        <div class="contents-container mb-3">
                            ...
                            <div class="card mb-2 shadow-sm content-item">
                                ...
                                <div class="card-body p-2">
                                    <input type="hidden" name="modules[{{ $module->id }}][contents][{{ $content->id }}][id]" value="{{ $content->id }}">

                                    <div class="mb-2">
                                        <label class="form-label">Content Title</label>
                                        <input type="text" name="modules[{{ $module->id }}][contents][{{ $content->id }}][title]" value="{{ $content->title }}" class="form-control" required>
                                    </div>

                                    <div class="mb-2">
                                        <label class="form-label">Video Type</label>
                                        <select name="modules[{{ $module->id }}][contents][{{ $content->id }}][video_type]" class="form-select">
                                            <option value="0" {{ $content->video_type==0?'selected':'' }}>Video File</option>
                                            <option value="1" {{ $content->video_type==1?'selected':'' }}>Video Link</option>
                                        </select>
                                    </div>

                                    <div class="mb-2">
                                        <label class="form-label">Upload File</label>
                                        <input type="file" name="modules[{{ $module->id }}][contents][{{ $content->id }}][file]" class="form-control">
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <input type="text" name="modules[{{ $module->id }}][contents][{{ $content->id }}][length]" value="{{ $content->length }}" class="form-control" placeholder="00:00:00">
                                    </div>
                                </div>
                            </div>
                            @endforeach
        ...
        <div class="d-flex justify-content-end gap-2">
            ...
            <button type="button" class="btn btn-outline-secondary px-4" onclick="window.history.back()">Cancel</button>
        </div>
    </form>
</div>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    function initCKEditor(textarea){
        ClassicEditor.create(textarea)
            .then(editor => { editors[textarea.name] = editor; })
    ...
    // Attach add content buttons for existing modules
