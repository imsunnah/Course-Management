                        ...
                        @if ($course->feature_video)
                            <div class="ratio ratio-16x9 rounded-3 shadow-lg overflow-hidden">
                                @if (Str::startsWith($course->feature_video, ['http', 'www']))
                                    <iframe src="{{ $course->feature_video }}" title="{{ $course->title }} Feature Video"
                    ...
                    <div class="col-lg-5">
                        ...
                        <ul class="list-group list-group-flush border rounded-3 mb-4 shadow-sm">
                            <li class="list-group-item d-flex justify-content-between align-items-center bg-light">
                                <span class="fw-semibold text-dark"><i class="fas fa-layer-group me-2 text-info"></i>
                                    Level:</span>
                                <span><span class="badge bg-secondary">{{ $course->level ?? 'N/A' }}</span></span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                <span class="fw-semibold text-dark"><i class="fas fa-tag me-2 text-info"></i>
                                    Category:</span>
                                <span>{{ $course->category->name ?? 'Uncategorized' }}</span>
                            </li>
                            <li
                                class="list-group-item d-flex justify-content-between align-items-center bg-secondary bg-opacity-10">
                                <span class="fw-bolder text-success"><i class="fas fa-dollar-sign me-2"></i> Price:</span>
                                <span
                                    class="h5 mb-0 text-success">{{ $course->course_price ? '৳' . $course->course_price : 'Free' }}</span>
                            </li>
                        </ul>
                    ...
                    <div class="accordion-item shadow-lg mb-4 border border-info rounded-3">
                        <h3 class="accordion-header" id="moduleHeading{{ $module->id }}">
                            ...
                            <button class="accordion-button **fs-4** fw-bold bg-info text-white **p-4** collapsed"
                                ...
                                aria-expanded="{{ $modIndex == 0 ? 'true' : 'false' }}"
